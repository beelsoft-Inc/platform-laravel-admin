...
`Tungnt\Admin\Widgets\Box` used to generate box components:

```php
use Tungnt\Admin\Widgets\Box;
...
`Tungnt\Admin\Widgets\Collapse` class used to generate folding components:
```php
use Tungnt\Admin\Widgets\Collapse;
...
`Tungnt\Admin\Widgets\Form` class is used to quickly build a form:
...
`Tungnt\Admin\Widgets\InfoBox` class is used to generate the information presentation block:

```php
use Tungnt\Admin\Widgets\InfoBox;
...
Refer to the section on the `InfoBox` in the `index()` method of the home page layout file [HomeController.php](/blob/master/src/Console/stubs/HomeController.stub).

## Tab component

`Tungnt\Admin\Widgets\Tab` class is used to generate the tab components:

```php
use Tungnt\Admin\Widgets\Tab;
...
`Tungnt\Admin\Widgets\Table` class is used to generate tables:

```php
use Tungnt\Admin\Widgets\Table;
